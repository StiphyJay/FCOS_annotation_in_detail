...
sys.path.remove('/home/sifan/slam-package/pointnet.pytorch-master') #移除非改环境的cv2对应的路径
import cv2, os
...
def main():
    ...
    parser.add_argument(    #模型参数加载
        "--config-file",
        default="/home/sifan/slam-package/FCOS/configs/fcos/fcos_R_50_FPN_1x.yaml",
        metavar="FILE",
        help="path to config file",
    )
    parser.add_argument(    #权重系数加载(模型加载)
        "--weights",
        default="/home/sifan/slam-package/FCOS/model/FCOS_imprv_R_50_FPN_1x.pth",
        metavar="FILE",
        help="path to the trained model",
    )
    parser.add_argument(    #测试图片路径
        "--images-dir",
        default="/home/sifan/slam-package/FCOS/demo/images",
        metavar="DIR",
        help="path to demo images directory",
    )
    parser.add_argument(    #最小图像尺寸
        "--min-image-size",
        type=int,
        default=800,
    ...
    parser.add_argument(    #在命令行里修改模型配置选项，默认为不修改
        "opts",
    ...
    demo_im_names = os.listdir(args.images_dir) #测试的图片路径

    # prepare object that handles inference plus adds predictions on top of image
    coco_demo = COCODemo(
        cfg, #配置参数
        confidence_thresholds_for_classes=thresholds_for_classes, #类别的置信度阈值
        min_image_size=args.min_image_size  #800
    )
