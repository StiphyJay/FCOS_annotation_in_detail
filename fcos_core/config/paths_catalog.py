...
class DatasetCatalog(object):
    ...
    def get(name):
        if "coco" in name:
            ...
            attrs = DatasetCatalog.DATASETS[name] #直接读取出对应的数据集的图片路径和标签文件
            args = dict(
                root=os.path.join(data_dir, attrs["img_dir"]), #图片路径
                ann_file=os.path.join(data_dir, attrs["ann_file"]),  #标签文件
            )
            return dict(
                factory="COCODataset",
                args=args,
